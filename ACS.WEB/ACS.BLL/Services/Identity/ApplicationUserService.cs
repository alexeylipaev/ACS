using ACS.BLL.DTO; using ACS.BLL.Infrastructure; using ACS.DAL.Entities; using System.Threading.Tasks; using Microsoft.AspNet.Identity; using System.Security.Claims; using ACS.BLL.Interfaces; using ACS.DAL.Interfaces; using System.Collections.Generic; using System.Linq; using System; using ACS.BLL.BusinessModels;  using System.Diagnostics; using System.Collections;  namespace ACS.BLL.Services {     public class ApplicationUserService :ServiceBase, IApplicationUserService     {         public ApplicationUserService(IUnitOfWork uow) : base(uow) { }
        ...
        public async Task<ApplicationRoleDTO> FindRoleByIdAsync(int roleId)         {             if (roleId == default(int))                 throw new ValidationException("Роль  не установлена", "");              var AppRole = await Database.RoleManager.FindByIdAsync(roleId);
            ...
            if (AppRole == null)                 throw new ValidationException("Роль не найдена", "");              //var mapper = new MapperConfiguration(cfg => cfg.CreateMap<ApplicationRole, ApplicationRoleDTO>()).CreateMapper();              var result = MapApplicationRole.RoleToRoleDTO(AppRole);             return result;         }
        ...
        //}          public bool IsInRole(string userName, string role)         {             bool result = false;             var applicationUser = Database.UserManager.FindByName(userName);              if (applicationUser != null)             {                 var AppRole = Database.RoleManager.FindByName(role);                 if (AppRole != null)                     return AppRole.Users.Any(u => u.UserId == applicationUser.Id);             }             return result;         }          public async Task<ApplicationUserDTO> FindByNameAsync(string userName)         {             if (string.IsNullOrEmpty(userName))                 throw new ValidationException("Имя пользователя не установлено", "");              var appUser = await Database.UserManager.FindByNameAsync(userName);             if (appUser == null)                 throw new ValidationException("Пользователь не найден", ""); 
            var result = MapApplicationUser.UserToUserDTO(appUser);             return result;         }          public async Task<ApplicationUserDTO> FindByEmailAsync(string Email)         {             if (string.IsNullOrEmpty(Email))                 throw new ValidationException("Почта пользователя не установлена", "");              var appUser = await Database.UserManager.FindByEmailAsync(Email);             if (appUser == null)                 throw new ValidationException("Пользователь не найден", "");               var result = MapApplicationUser.UserToUserDTO(appUser);             return result;         }          public ApplicationUserDTO FindByEmail(string Email)         {             if (string.IsNullOrEmpty(Email))                 throw new ValidationException("Почта пользователя не установлена", "");              var Users = Database.UserManager.Users.ToList();              ApplicationUser appUser = (from user in Users                            where user.Email == Email                            select user).FirstOrDefault();              if (appUser == null)                 throw new ValidationException("Пользователь не найден", "");              var result = MapApplicationUser.UserToUserDTO(appUser);             return result;         }
...
          public async Task<OperationDetails> AddLoginAsync(int userId, UserLoginInfo login)         {             var result = await Database.UserManager.AddLoginAsync(userId, login);              if (result.Errors.Count() > 0)                 return new OperationDetails(false, result.Errors.FirstOrDefault(), "");              return new OperationDetails(true, result.Errors.FirstOrDefault(), "");         }
        ...
        public async Task<OperationDetails> CreateOrUpdateAsync(ApplicationUserDTO UserDTO)         {             ApplicationUser EditableObj = await Database.UserManager.FindByIdAsync(UserDTO.id);
            var UpdateAppUser = MapApplicationUser.UserDTOToUser(UserDTO);
        ...
        public Task<OperationDetails> CreateRoleAsync(ApplicationRoleDTO applicationRoleDTO)
        {
            throw new NotImplementedException();
        }

        public Task<OperationDetails> UpdateRoleAsync(ApplicationRoleDTO applicationRoleDTO)
        {
            throw new NotImplementedException();
        }

        public Task<OperationDetails> DeleteRoleAsync(int id)
        {
            throw new NotImplementedException();
        }
...
             var result = MapApplicationUser.UserToUserDTO(appUser);

            ApplicationUser user = await Database.UserManager.FindByIdAsync(id);

            var userDto = MapApplicationUser.UserToUserDTO(user);
