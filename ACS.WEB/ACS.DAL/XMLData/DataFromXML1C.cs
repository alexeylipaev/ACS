...
{
    static private ACSContext context { get; set; }
    ...
    {
        IEnumerable<string> AdminsEmails = new List<string>(){ "kachkov@dinamika-avia.ru", "lipaev@dinamika-avia.ru" };
    ...
    public static void GenerateUserRepository(ACSContext Context)
    {
        ...
        foreach (var dataUser in query)
        {
            ...
            if (dataUserAD.UserPrincipal != null)
            {
                newDataUser.SID = dataUserAD.SID;
                newDataUser.Email = dataUserAD.Email;

                //AppUser = new ApplicationUser();
                //AppUser.Email = newDataUser.Email;
                //AppUser.UserName = dataUserAD.UserPrincipal.DisplayName;
                //AppUser.Employee = newDataUser;
                //newDataUser.ApplicationUser = AppUser;
            }
            ...
            //{
            //    AppUser.Roles.Add(new AppUserRole() { UserId = AppUser.Id, RoleId = 2 });
            //}
            //else
            //{
            //    AppUser.Roles.Add(new AppUserRole() { UserId = AppUser.Id, RoleId = 1 });
            //}
    ...
    public static void GeneratePostsEmployeesСode1СRepository(ACSContext Context)
    {
        ...
        foreach (var empl in query)
        {
            ...
            var PostsEmployeesСode1С = new PostEmployeeСode1С()
            {
    ...
    public static void GenerateWorkHistoryRepository(ACSContext Context)
    {
        ...
        foreach (var WorkHistory in query)
        {
            ...
            //КодДолжности1С
            PostEmployeeСode1С PUC = Context.PostsEmployeesСode1С.FirstOrDefault
